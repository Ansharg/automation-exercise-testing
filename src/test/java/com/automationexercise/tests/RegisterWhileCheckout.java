...
public class RegisterWhileCheckout extends BaseClassTest {
	
	@Test(dataProvider="testData")
	public void registerWhileCheckout(HashMap<String,String> info) throws InterruptedException {
		...
		if(driver.getCurrentUrl().equals("https://www.automationexercise.com/view_cart")) {			
			LoginRegister signupPage = cartPage.register();
			SignUpPage signup = signupPage.signup(info.get("name"),info.get("email"));
			if(getElementsByXPath("//p[normalize-space()='Email Address already exist!']").size()==1) {
				signupPage.login(info.get("email"),info.get("password"));
			}
			else {			
				signup.fillInfo(info);
			}
			registerWhileCheckout(info);
		}
		else {
